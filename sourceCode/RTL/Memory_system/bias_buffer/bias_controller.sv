...
always_comb begin
    ...
    if (bias_valid && (bias_type == 2'b00)) begin
        next_counter = 6'd1;     // start counting
    end
    else if (bias_valid_reg) begin
        next_counter = counter + 1;
    end else if (next_counter == 6'd8)begin
        next_counter = 0;
    end
end
...
always_ff @(posedge clk ) begin
    if(!reset_n) begin
        ...
        // stop after 8 cycles
        if((bias_type != BUFFER_FULL && next_counter == 6'd8 ) ||(bias_type == BUFFER_FULL && next_counter == 6'd63 ) ) begin
            bias_valid_reg <= 0;
           // next_counter <= 0;
        end
    end
...
always_comb begin
  ...
  unique  case(stateA)
        ...
        BUFFER_LOAD:begin
            if(stateC != BUFFER_STREAM && stateB != BUFFER_STREAM && loadcounterA == 4'b0111)begin
